...
export async function generateWebsiteContent(
  contents: ContentGenerationRequest[]
 ...
 */
export async function generateHeadingStructure(
  keyword: string,
  contentType: string
): Promise<string[]> {
  const API_KEY = process.env.OPENAI_API_KEY
  ...
  const systemPrompt =
    'You are an SEO content strategist. Generate an optimized heading structure (H1, H2, H3) for the given content type and keyword. Return as a JSON array of strings with full headings.'

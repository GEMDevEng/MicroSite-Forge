...
// Assignment recommendation with score
interface AssignmentRecommendation extends TeamMember {
  score: number
}
...
export class LeadScorer {
  static calculateScore(incomingLead: IncomingLead): LeadScore {
    ...
    const sourceScore = source.includes('paid')
      ? 'paid'
    ...
    const totalScore = Math.round(
      (engagement + intentLevel + budgetIndicators + timelineSignals) / 4
    )
...
export class LeadEnrichment {
  async enrichLead(leadId: string): Promise<void> {
    try {
      ...
      if (leadError || !lead) {
        logger.error(
          'Failed to fetch lead for enrichment',
          leadError || new Error('Unknown error'),
          { leadId }
        )
        return
      }
...
export class LeadRouter {
  ...
  // Helper method to get available team members
  private static getAvailableTeamMembers(): Promise<TeamMember[]> {
  ...
  private static findBestAssignee(
    ...
    if (this.isHighQualityLead(leadData.score)) {
      const highCapacityMembers = availableMembers.filter(
        (member) => member.capacity >= 15 && member.performance_score >= 85
      )
    ...
    const highPerformingMembers = availableMembers.filter(
      (member) => member.performance_score >= 85
    )
  ...
  private static selectByCapacity(members: TeamMember[]): TeamMember {
  ...
  private static roundRobinAssign(availableMembers: TeamMember[]): TeamMember {
  ...
  private static isSameLocation(leadLocation: string, memberLocation: string): boolean {
    ...
    // Check if both locations belong to the same region
    for (const locations of Object.values(locationMappings)) {
      const normalizedLocations = locations.map((loc) => normalizeLocation(loc))
      if (
        normalizedLocations.includes(leadNormalized) &&
        normalizedLocations.includes(memberNormalized)
      ) {
        return true
      }
    }
  ...
  private static hasRelevantSkills(leadData: LeadData, memberSkills: string[]): boolean {
    ...
    if (
      leadIndustry.includes('ecommerce') &&
      (relevantSkills.includes('ecommerce') || relevantSkills.includes('sales'))
    )
      return true
    if (
      leadIndustry.includes('marketing') &&
      (relevantSkills.includes('marketing') || relevantSkills.includes('digital-marketing'))
    )
      return true
  ...
  static async getAssignmentRecommendations(
    ...
    try {
      ...
      const scoredMembers: AssignmentRecommendation[] = availableMembers.map((member) => ({
        ...member,
        score: this.calculateAssignmentScore(leadData, member),
      }))

      const recommendations = scoredMembers.sort((a, b) => b.score - a.score).slice(0, 3)
  ...
  private static calculateAssignmentScore(leadData: LeadData, member: TeamMember): number {
    let score = 0
    ...
    const hoursSinceLastAssignment =
      (Date.now() - new Date(member.last_assignment).getTime()) / (1000 * 60 * 60)
  ...
  static async updateTeamMemberCapacity(
    memberId: string,
    assignedLeadsChange: number
  ): Promise<void> {
    try {
      ...
      logger.error(
        'Failed to update team member capacity',
        error instanceof Error ? error : new Error('Unknown error'),
        { memberId }
      )
    }
  ...
  private static async checkCapacityAlert(memberId: string): Promise<void> {
    try {
      ...
      if (mockUtilization > 0.9) {
        logger.warn('Team member at risk of overload', {
          memberId,
          utilization: Math.round(mockUtilization * 100),
        })
      }
    } catch (error) {
      logger.error(
        'Failed to check capacity alert',
        error instanceof Error ? error : new Error('Unknown error'),
        { memberId }
      )
    }
  ...
  async createLead(incomingLead: IncomingLead): Promise<LeadData> {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        status: 'new',
        score_data: JSON.stringify(score) as Json,
        contact_info: JSON.stringify(contactInfo) as Json,
        tags: [],
      })
      .select()
    ...
    if (!data) {
      logger.error('No data returned from lead creation', new Error('No data returned'), {
        lead: incomingLead,
      })
      throw new Error('Failed to create lead')
    }
